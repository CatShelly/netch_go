...
extern "C" {
	...
	__declspec(dllexport) BOOL __cdecl aio_dial(int name, LPWSTR value)
	{
		...
		case AIO_DNSPROX:
			...
			break;
		case AIO_DNSDOMAINONLY:
			dnsDomainOnly = (wstring(value).find(L"false") == string::npos);
			break;
		case AIO_DNSHOST:
			dnsHost = ws2s(value);
			break;
		...
		case AIO_CLRNAME:
			...
			break;
		case AIO_CLRDNSRULE:
			dnsDomainRules.clear();
			break;
		case AIO_ADDDNSRULE:
		{
			auto rule = ws2s(value);
			if (rule.empty())
			{
				return FALSE;
			}
			transform(rule.begin(), rule.end(), rule.begin(), [](unsigned char ch) { return static_cast<char>(tolower(ch)); });
			dnsDomainRules.emplace_back(rule);
			break;
		}
		case AIO_BYPNAME:
			try
